...
export const registerCommands = async () => {
    ...
    const rest = new REST({ version: "9" }).setToken(
        typeof process.env["BOT_TOKEN"] == "string" ? process.env["BOT_TOKEN"] : ""
    );

    try {
        console.log("Refreshing slash commands...");
        // Add the commands using commandsInit array
        await rest.put(Routes.applicationCommands(typeof process.env["CLIENT_ID"] == "string" ? process.env["CLIENT_ID"] : ""), {
            body: commandsInit,
        });
    } catch (err) {
        // Catch error
 ...
 * @param path full path to search through
 */
export const registerEvents = async (path = join(__dirname, "..", "events")) => {
